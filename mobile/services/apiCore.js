 ...
 * PATH: vulnassess-app/services/apiCore.js
 */
...
export function createApi(storage) {
  ...
  return {
    ...
    downloadPDF: async (id) => {
      const res = await fetch(`${BASE_URL}/reports/${id}/pdf`, { headers: await getHeaders() });
    ...
    getReportDownloadToken: (scanId) => post(`/reports/${scanId}/download-token`),

    downloadAIPDF: async (scanId) => {
      const res = await fetch(`${BASE_URL}/reports/${scanId}/ai-remediation/pdf`, { headers: await getHeaders() });
      if (!res.ok) return null;
      return res.blob();
    },
